...
to run all available tests.  Use the flag "-h" to see driver script
options.  To change default options, edit the "nosetests" section in
setup.cfg.  To disable output capture, add the flag "-s" when invoking
nose.  To run only one of the modules under the ``tests`` directory,
give that module's name when invoking nose.
