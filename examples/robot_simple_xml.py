...
code by Petter Nilsson and Nok Wongpiromsarn.  It demonstrates basic
writing and reading of a tulipcon XML file.  Toggle the truth value of
load_from_XML to indicate whether to generate a new tulipcon XML file,
or read from one.
...
if not load_from_XML:
    ...
    # spec = GRSpec(sys_init='X0reach',
    #               env_prog='!park',
    #               sys_safety='next(X0reach) = ((X0 | X0reach) & !park)',
    #               sys_prog=['X5', 'X0reach'])
    ...
    # prob.createProbFromContDynamics(env_vars=env_vars, sys_disc_vars=sys_disc_vars,
    #                                 cont_state_space=cont_state_space,
    #                                 cont_props=cont_props,
    #                                 sys_dyn=sys_dyn, spec=spec, verbose=2)
